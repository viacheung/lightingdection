...
$(function () {
    ...
    $('#btnSave').bind('click', function () {
        ...
        var isError = document.getElementById("input_isError").value;
        //var msg="";

        if (method.localeCompare("add") == 0) {
            $.ajax({
                url: "addPanDuan",
                data: {"datameaningId": datameaningId, "name": name, "data": data, "isError": isError},
                dataType: 'json',
                type: 'post',
...
function del(selected) {
    $.messager.confirm("确认", "确认要删除吗？", function (yes) {
        if (yes) {
            var sd = 'deletePanDuan';
            var data = selected.data;
            var datameaningId = selected.datameaningId;
            $.ajax({
                url: sd,
                data: {"data": data, "datameaningId": datameaningId},
                dataType: 'json',
                type: 'post',
                success: function (rtn) {
